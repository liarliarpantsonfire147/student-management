...
alter table public.grades add column if not exists gpa numeric(3,2);
alter table public.grades drop constraint if exists grades_gpa_check;
alter table public.grades add constraint grades_gpa_check check (gpa is null or (gpa >= 0 and gpa <= 4));
...
returns table(student_id uuid, full_name text, email text, year_group text, grade text, gpa numeric, feedback text, updated_at timestamptz)
language plpgsql security definer set search_path = public as $$
begin
  if not public.teaches_class(target_class) then raise exception 'You do not have access to this class'; end if;
  return query select s.id, s.full_name, s.email, s.year_group, g.grade, g.gpa, g.feedback, g.updated_at
...
drop function if exists public.get_class_metrics();
...
language sql security definer set search_path = public as $$
  select c.id, c.name, c.year_group, count(distinct e.student_id), count(g.id), round(avg(coalesce(g.gpa, public.grade_points(g.grade))), 2)
...
language sql security definer set search_path = public as $$
  select s.id, s.full_name, s.email, s.year_group, count(distinct e.class_id), round(avg(coalesce(g.gpa, public.grade_points(g.grade))), 2)
